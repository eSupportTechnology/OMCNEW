...
@section('dashboard-content')
    ...
    <div class="d-flex justify-content-between align-items-center mt-4">
        <div class="button-tabs">
            <button class="tab-button mb-1 {{ $tab == 'all-orders' ? 'active' : '' }}" data-target="all-orders">All
                Orders</button>
            <button class="tab-button mb-1 {{ $tab == 'to_be_shipped' ? 'active' : '' }}"
                data-target="in-progress-orders">In Progress</button>
            <button class="tab-button mb-1 {{ $tab == 'shipped' ? 'active' : '' }}"
                data-target="shipped-orders">Shipped</button>
            <button class="tab-button" data-target="delivered-orders">Delivered</button>
            <button class="tab-button mb-1 {{ $tab == 'confirmed' ? 'active' : '' }}"
                data-target="confirmed-orders">Confirmed</button>
        </div>
        ...
        @foreach($orders as $order)
            <div class="order-card">
                <div class="order-card-header">
                    ...
                    <a href="{{ route('myorder-details', ['order_code' => $order->order_code]) }}"
                        class="order-details-link">Order Details ></a>
                </div>
                    ...
                    <div class="order-image" style="position: relative;">
                        ...
                        @if($additionalCount > 0)
                            <span class="additional-count"
                                style="position: absolute; top: 58px; right: 20px; background: rgba(0, 0, 0, 0.3); color: white; padding: 5px; border-radius: 5px;">+{{ $additionalCount }}</span>
                        @endif
                    </div>
                    ...
                    <div style="text-align: right; margin-top: 5px;">
                        ...
                        @elseif($order->status === 'Shipped')
                            <a href="javascript:void(0);" class="btn-confirm"
                                onclick="openConfirmDeliveryModal('{{ $order->order_code }}')">Confirm Delivery</a>
                            <br>
                            <a href="#" class="btn-cancel mt-1" onclick="openCancelModal('{{ $order->order_code }}')">Cancel</a>
                        @endif
                    </div>
    ...
    <div class="modal fade" id="cancel-confirmation-modal" tabindex="-1" role="dialog" aria-labelledby="confirmModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" id="confirm-cancel" class="btn btn-success"
                        style="font-size: 13px">Confirm</button>
    ...
    <script>
        document.querySelectorAll('.tab-button').forEach(btn => {
            btn.addEventListener('click', function () {
                const targetTab = this.getAttribute('data-target').replace('-orders', '');
                window.location.href = "{{ route('myorders') }}" + "?tab=" + targetTab;
            });
        });
    </script>
