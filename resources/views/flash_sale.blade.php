...
@section('content')
                        ...
                        @if ($currentDateTime->lessThanOrEqualTo($endDateTime))
                            <div class="col-6 col-sm-4 col-md-2 col-lg- mb-3">
                                <div class="sale-item position-relative">
                                    <a href="{{ route('product-description', ['product_id' => $sale->product->product_id]) }}"
                                        class="d-block text-decoration-none">
                                        <div class="sale-image-wrapper position-relative">
                                            @if ($sale->product->images->isNotEmpty())
                                                <img src="{{ asset('storage/' . $sale->product->images->first()->image_path) }}"
                                            ...
                                            <button type="button"
                                                ...
                                                data-bs-target="#cartModal_{{ $sale->product->product_id }}">
                                                <i class="bi bi-cart-plus"></i>
                                            </button>
                                        </div>
                                        <h6 class="product-name">
                                            {{ \Illuminate\Support\Str::limit($sale->product->product_name, 25, '...') }}
                                        </h6>
                                        <div class="price">
                                            <span class="offer-price">Rs. {{ number_format($sale->sale_price, 2) }}</span>
                                            <s style="font-size: 14px; color: #989595; font-weight:500">Rs.
                                                {{ number_format($sale->normal_price, 2) }}</s>
                                        </div>
                                    </a>
                                </div>
                            </div>
    ...
    <!-- cart modal -->
    @foreach ($sales as $sale)
        <div class="modal fade" id="cartModal_{{ $sale->product->product_id }}" tabindex="-1"
                        ...
                        <div class="row gx-5">
                            <aside class="col-lg-5">
                                <div class="rounded-4 mb-3 d-flex justify-content-center">
                                    <a class="rounded-4 main-image-link"
                                        href="{{ asset('storage/' . $sale->product->images->first()->image_path) }}">
                                        <img id="mainImage" class="rounded-4 fit"
                                            src="{{ asset('storage/' . $sale->product->images->first()->image_path) }}" />
                                    </a>
                                </div>
                            ...
                            <main class="col-lg-7">
                                ...
                                <div class="product-container">
                                    ...
                                    @auth
                                        <a href="#"
                                            class="btn btn-custom-cart mb-3 add-to-cart-modal shadow-0 {{ $sale->product->quantity <= 1 ? 'btn-disabled' : '' }}"
                                            data-product-id="{{ $sale->product->product_id }}" data-auth="true"
                                            style="width: 40%;">
                                    ...
                                    @else
                                        <a href="#"
                                            class="btn btn-custom-cart mb-3 add-to-cart-modal shadow-0 {{ $sale->product->quantity <= 1 ? 'btn-disabled' : '' }}"
                                            data-product-id="{{ $sale->product->product_id }}" data-auth="false"
                                            style="width: 40%;">
                                            <i class="me-1 fa fa-shopping-basket"></i>Add to cart
                                        </a>
                                    @endauth
                                ...
                                <a href="{{ route('product-description', $sale->product->product_id) }}"
