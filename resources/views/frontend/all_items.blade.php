                            ...
                            <div class="col-lg-4 col-md-4">
                                <div class="products-ordering">
                                    <select class="form-select sort-dropdown" onchange="applySorting(this.value)">
                                        ...
                                        <option value="rating_high_low" {{ request('sort') === 'rating_high_low' ? 'selected' : '' }}>Highest Rated</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        @if ($products->isEmpty())
                            <div class="col-12">
                                <div class="no-products text-center py-5">
                                    ...
                                    <p>Try adjusting your filters or <a href="{{ route('all-items') }}">clear all filters</a> to see more products.</p>
                                </div>
                            </div>
                        @else
                    ...
                    <div class="modal-body">
                        <div class="row gx-5">
                                ...
                                <div class="rounded-4 mb-3 d-flex justify-content-center">
                                    @if ($product->images->first())
                                        <a class="rounded-4 main-image-link" href="{{ asset('storage/' . $product->images->first()->image_path) }}">
                                            <img id="mainImage" class="rounded-4 fit" style="width:280px; height:auto" src="{{ asset('storage/' . $product->images->first()->image_path) }}" />
                                        </a>
                                    @else
                                        <a class="rounded-4 main-image-link" href="{{ asset('images/default.jpg') }}">
                                            <img id="mainImage" class="rounded-4 fit" src="{{ asset('images/default.jpg') }}" />
                                        </a>
                                    @endif
                                </div>
                            ...
                            <main class="col-lg-7">
                                ...
                                <p class="description">{{ str_replace('&nbsp;', ' ', strip_tags($product->product_description)) }}</p>
                                ...
                                <div class="product-container">
                                    ...
                                    @if ($product->variations->where('type', 'Color')->isNotEmpty())
                                        <div class="mb-2">
                                            ...
                                            @foreach ($product->variations->where('type', 'Color') as $color)
                                                @if ($color->quantity > 0)
                                                    <button class="btn btn-outline-secondary btn-sm color-option" style="background-color: {{ $color->hex_value }}; border-color: #e8ebec; height: 17px; width: 15px;" data-color="{{ $color->hex_value }}" data-color-name="{{ $color->value }}">
                                                    </button>
                                                @endif
                                            @endforeach
                                        </div>
                                    ...
                                    @auth
                                        <a href="#" class="btn btn-custom-cart mb-3 add-to-cart-modal shadow-0 {{ $product->quantity <= 1 ? 'btn-disabled' : '' }}" data-product-id="{{ $product->product_id }}" data-auth="true" style="width: 40%; background-color: #007bff; color: white;">
                                            <i class="me-1 fa fa-shopping-basket"></i>Add to cart
                                        </a>
                                    @else
                                        <a href="#" class="btn btn-custom-cart mb-3 add-to-cart-modal shadow-0 {{ $product->quantity <= 1 ? 'btn-disabled' : '' }}" data-product-id="{{ $product->product_id }}" data-auth="false" style="width: 40%; background-color: #007bff; color: white;">
                                            <i class="me-1 fa fa-shopping-basket"></i>Add to cart
                                        </a>
                                    @endauth
                                </div>
